...
void Preprocess::ouster_handler(const sensor_msgs::PointCloud2::ConstPtr &msg, PointCloudXYZI::Ptr &pcl_out)
{
  ...
  //对于128线的雷达
  // int skid_line = 2;//need to skid the max and min `skid_line` 
  // if((static_cast<int>(pl_orig.points[i].ring) < skid_line || static_cast<int>(pl_orig.points[i].ring) > (127 - skid_line) ) ||
  // ((static_cast<int>(pl_orig.points[i].ring) % 8) != 0 )) 
  //     continue;
